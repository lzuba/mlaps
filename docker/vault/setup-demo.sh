...
if [ ! -f "/shared/INITIALIZED" ]; then
    ...
    vault write -field=certificate pki/root/generate/internal \
      common_name="com.foobar.mlaps" \
      ttl=45600h \
      key_bits=4096 > /tmp/shared/ca.crt

    openssl req -nodes -newkey rsa:2048 -keyout /tmp/shared/web.key -out /tmp/shared/web.csr \
        -subj "/C=fo/ST=baz/L=bar/O=foobar/OU=MLAPS/CN=mlaps.foobar.com"

    # get a server certificate
    vault write -field=certificate pki/sign/mlaps common_name="mlaps.foobar.com" csr=@/tmp/shared/web.csr ttl=8808h > /tmp/shared/web.crt
